...
(() => {
  ...
  window.addEventListener('load',()=>setTimeout(install,250)); window.toonBubbleManual={renderManual,renderManualSidebar,drawManualOverlay};
})();
</script>
'''
...
def add_detection(results,bbox,text,conf,tile_y0,scale_x,scale_y,image_w,image_h):
    ...
    x0=int(round(float(np.min(pts[:,0]))/scale_x)); y0=int(round(float(np.min(pts[:,1]))/scale_y+tile_y0)); x1=int(round(float(np.max(pts[:,0]))/scale_x)); y1=int(round(float(np.max(pts[:,1]))/scale_y+tile_y0));
    x0=max(0,min(image_w-1,x0)); y0=max(0,min(image_h-1,y0)); x1=max(0,min(image_w,x1)); y1=max(0,min(image_h,y1)); bbox_out=[x0,y0,x1,y1]
...
def fit_polygon_text(draw,text,poly,bbox):
    ...
    for size in range(max_size,7,-1):
        f=ImageFont.truetype(font_path,size); spacing=max(2,size//5); words=text.split(); lines=[]; cur=''; y_cursor=miny+size
        for word in words:
            test=word if not cur else cur+' '+word; test_w=draw.textbbox((0,0),test,font=f)[2]
            allowed=polygon_width_at(inner,y_cursor,x0,x1)-16
...
async def render(file:UploadFile=File(...),items_json:str=Form(...)):
    ...
    for item in items:
        ...
        block='\n'.join(lines); bb=draw.multiline_textbbox((0,0),block,font=font,spacing=spacing,align='center'); tw,th=bb[2]-bb[0],bb[3]-bb[1]; tx=x0+(x1-x0-tw)/2; ty=y0+(y1-y0-th)/2
