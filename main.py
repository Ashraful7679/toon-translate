...
async def serve_index():
    ...
    # Keep index.html untouched while adding the manual bubble workflow.
    html += r'''
<script>
(() => {
  const state = { manual: {}, mode: 'bubble' };
  const key = () => String(window.currentIndex ?? -1);
  ...
  function addToolbar() {
    ...
    box.innerHTML = '<button id="bubbleModeBtn" class="btn btn-primary" style="padding:7px 11px;font-size:12px">💬 Bubble Select: ON</button>' +
      '<button id="clearBubbleBtn" class="btn" style="padding:7px 11px;font-size:12px">🗑️ Clear Bubbles</button>';
  ...
  async function selectBubble(ev) {
    ...
    try {
      ...
      if (!exists) {
        items().push({id: Date.now(), text:'', conf:1, bbox:data.bbox, polygon:data.polygon || [], translation:'', manual:true});
      }
      renderManualSidebar(); drawManualOverlay();
    } catch (e) {
      console.warn(e);
  ...
  function drawManualOverlay() {
    ...
    items().forEach((it,i) => {
      ...
      rect.setAttribute('class','bbox-rect active'); rect.style.fill='rgba(16,185,129,.10)'; rect.style.stroke='#10b981';
      rect.addEventListener('click', e => { e.stopPropagation(); focusCard(i); });
      const t = document.createElementNS('http://www.w3.org/2000/svg','text');
      t.setAttribute('x',x0+5); t.setAttribute('y',Math.max(14,y0+15)); t.setAttribute('class','bbox-badge'); t.textContent=String(i+1);
  ...
  function focusCard(i) {
    ...
    if(el){ el.scrollIntoView({behavior:'smooth',block:'center'}); const inp=el.querySelector('textarea'); if(inp) inp.focus(); }
  }
  ...
  function renderManualSidebar() {
    ...
    if(!list.length){
      wrap.innerHTML='<div class="empty-state"><div class="empty-icon">💬</div><p>Bubble Select চালু রেখে ছবির প্রতিটি পূর্ণ বাবলের ভিতরে একবার ক্লিক করুন। অসম্পূর্ণ বাবল ক্লিক করবেন না।</p></div>';
      return;
    }
    wrap.innerHTML='';
    list.forEach((it,i)=>{
      const card=document.createElement('div'); card.className='item-card'; card.id='bubble-card-'+i;
      card.innerHTML=`<div class="item-header"><span class="item-badge">Bubble ${i+1}</span><div class="item-actions"><button class="btn-icon" title="এই বাবল মুছুন">🗑️</button></div></div>`+
        `<div class="original-text">Manual bubble selection • ${it.bbox[2]-it.bbox[0]} × ${it.bbox[3]-it.bbox[1]} px</div>`+
        `<div class="input-wrapper"><textarea class="translation-input" placeholder="এখানে বাংলা অনুবাদ লিখুন...">${it.translation || ''}</textarea></div>`+
        `<div class="input-tip"><span>Enter = পরের বাবল</span><span>Render করলে লেখা বাবলের মধ্যে Auto-Fit হবে</span></div>`;
      const ta=card.querySelector('textarea');
      ta.addEventListener('input',()=>{it.translation=ta.value;});
      ta.addEventListener('keydown',e=>{ if(e.key==='Enter' && !e.shiftKey){ e.preventDefault(); const n=wrap.querySelectorAll('textarea')[i+1]; if(n)n.focus(); }});
      card.querySelector('.btn-icon').onclick=()=>{ list.splice(i,1); renderManualSidebar(); drawManualOverlay(); };
  ...
  async function renderManual() {
    ...
    try{
      ...
      const img=document.getElementById('mainImage');
      if(window.activeView === 'rendered' || document.getElementById('btnRendered')?.classList.contains('active')) img.src=url;
      if(window.switchView) window.switchView('rendered');
      else img.src=url;
    }catch(e){ alert('Render করা যায়নি: '+e.message); }
  ...
  function install() {
    addToolbar();
    const img=document.getElementById('mainImage');
    if(img && !img.dataset.bubbleBound){ img.dataset.bubbleBound='1'; img.addEventListener('click',selectBubble); }
    ...
    const observer=new MutationObserver(()=>{ addToolbar(); const im=document.getElementById('mainImage'); if(im&&!im.dataset.bubbleBound){im.dataset.bubbleBound='1';im.addEventListener('click',selectBubble);} });
...
def detect_text_with_easyocr(img,reader):
    ...
    for tile_y0,tile_y1 in tiles:
        ...
        for variant_index,variant in enumerate(preprocess_variants(tile)):
            ...
            try:
                results=reader.readtext(variant,paragraph=False,detail=1,text_threshold=0.35,low_text=0.25,link_threshold=0.30,mag_ratio=1.0,canvas_size=2500,slope_ths=0.45,ycenter_ths=0.6,height_ths=0.6,width_ths=0.6,add_margin=0.05,decoder="greedy")
            except Exception as err:
                print(f"EasyOCR error tile={tile_y0}:{tile_y1}, variant={variant_index}: {err}"); continue
...
async def detect(file: UploadFile = File(...)):
    ...
    h,w=img.shape[:2]; print(f"OCR image received: {w}x{h}")
...
async def select_bubble(file: UploadFile = File(...), x: int = Form(...), y: int = Form(...)):
    """Find the speech-bubble region nearest a click. This is deliberately separate from OCR.
    The user chooses which four complete bubbles count; incomplete bubbles never need to enter OCR."""
    contents=await file.read(); img=cv2.imdecode(np.frombuffer(contents,np.uint8),cv2.IMREAD_COLOR)
    if img is None:raise HTTPException(status_code=400,detail="Invalid image")
    h,w=img.shape[:2]; x=max(0,min(w-1,int(x))); y=max(0,min(h-1,int(y)))
    gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    gray=cv2.GaussianBlur(gray,(3,3),0)

    # If the user clicks on a letter, move to the nearest bright interior pixel.
    best=None; best_val=-1
    for radius in (8,16,28):
        yy0,yy1=max(0,y-radius),min(h,y+radius+1); xx0,xx1=max(0,x-radius),min(w,x+radius+1)
        patch=gray[yy0:yy1,xx0:xx1]
        loc=np.argwhere(patch>=205)
        if loc.size:
            for py,px in loc:
                val=int(patch[py,px]); dist=(py+yy0-y)**2+(px+xx0-x)**2
                score=val-dist*0.15
    ...
    mask=(flood[1:-1,1:-1]>0).astype(np.uint8)*255
    ...
    pad=max(3,int(min(bw,bh)*0.02)); bx=max(0,bx-pad); by=max(0,by-pad); bw=min(w-bx,bw+2*pad); bh=min(h-by,bh+2*pad)
    eps=max(2.0,min(bw,bh)*0.01)
    approx=cv2.approxPolyDP(contour,eps,True).reshape(-1,2).tolist()
...
async def auto_translate(req: AutoTranslateRequest):
    ...
    for text in req.texts:
        ...
        if not translated:
            try:
                import requests
                r=requests.get("https://api.mymemory.translated.net/get",params={"q":text,"langpair":"en|bn"},timeout=6)
...
async def render_image(file: UploadFile = File(...), items_json: str = Form(...)):
    ...
    # Clear each selected bubble. Polygon fill is used for manual bubble selections,
    # so the white area follows the actual bubble instead of a rectangular OCR box.
    for item in items:
        ...
        if polygon and len(polygon)>=3:
            ...
            x0,y0,x1,y1=map(int,bbox); pad=4
            cv2.rectangle(img_cv,(max(0,x0-pad),max(0,y0-pad)),(min(img_cv.shape[1],x1+pad),min(img_cv.shape[0],y1+pad)),(255,255,255),-1)
    ...
    for item in items:
        ...
        x0,y0,x1,y1=map(int,bbox); W=max(10,x1-x0); H=max(10,y1-y0)
        # Larger bubbles get larger type. Then reduce until BOTH width and height fit.
        fontsize=min(96,max(12,int(H*0.34))); font=None; lines=[]
        while fontsize>=10:
